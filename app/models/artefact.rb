...
#  validates_attachment_content_type :ingestedFileRelationshipList, content_type: ["/\Atext\/.*\Z/", "application/json"]
  ...
  has_many :out, :creator, rel_class: :Is_Creator


#  has_many :in_or_out_or_both, :deviceRecordedAt, type: :FILL_IN_RELATIONSHIP_TYPE_HERE
#  has_many :in_or_out_or_both, :occurredAt, type: :FILL_IN_RELATIONSHIP_TYPE_HERE
  has_many :out, :ingestedAt, rel_class: :Absolutedatetime_Ingested_At
  has_many :out, :referencesAbsoluteDatetimes, rel_class: :References_Absolutedatetime
  has_many :out, :editedAt, rel_class: :Absolutedatetime_Edited_At
...
#  has_many :out, :datetime_occurred_at, type: :ABSOLUTEDATETIME_OCCURREDAT, model_class: :AbsoluteDateNode
#  has_many :out, :datetime_occurred_at, type: :DATETIME_ARTEFACT_RECORDED_AT, model_class: :AbsoluteDateNode
#  has_many :out, :datetimeOccurredAt, type: :DATETIME_ARTEFACT_RECORDED_AT, model_class: :AbsoluteDateNode
#  has_many :out, :datetimeRecordedAt, type: :DATETIME_ARTEFACT_RECORDED_AT, model_class: :AbsoluteDateNode
  has_many :out, :absoluteDatetimeOccurredAt, rel_class: :Absolutedatetime_Occurred_At
  has_many :out, :absoluteDatetimeRecordedAt, type: :ABSOLUTEDATETIME_RECORDED_AT, model_class: :AbsoluteDateNode

  has_many :out, :geospatialOccurrededAt, type: :GEOSPATIAL_OCURRED_AT, model_class: :GeospatialNode
  has_many :out, :geospatialRecordedAt, type: :GEOSPATIAL_RECORDED_AT, model_class: :GeospatialNode #Technically for recordings, but Dayone entries are recordings, for example
...
class Is_Creator
  include Neo4j::ActiveRel
  # Making sure that only one relationship between the same two nodes is created
...
class Absolutedatetime_Ingested_At
  include Neo4j::ActiveRel
  # Making sure that only one relationship between the same two nodes is created

  from_class :Artefact
  to_class   :AbsoluteDateNode
  type 'ABSOLUTEDATETIME_INGESTED_AT'

  creates_unique :all #only count it as a unique relationship when all properties match (and therefore don't duplicate it)
end

class References_Absolutedatetime
  include Neo4j::ActiveRel
  # Making sure that only one relationship between the same two nodes is created

  from_class :Artefact
  to_class   :AbsoluteDateNode
  type 'REFERENCES_ABSOLUTEDATETIME'

  creates_unique :all #only count it as a unique relationship when all properties match (and therefore don't duplicate it)
end


class Absolutedatetime_Edited_At
  include Neo4j::ActiveRel
  # Making sure that only one relationship between the same two nodes is created

  from_class :Artefact
  to_class   :AbsoluteDateNode
  type 'ABSOLUTEDATETIME_EDITED_AT'

  creates_unique :all #only count it as a unique relationship when all properties match (and therefore don't duplicate it)
end


class Absolutedatetime_Occurred_At
  include Neo4j::ActiveRel
  # Making sure that only one relationship between the same two nodes is created

  from_class :Artefact
  to_class   :AbsoluteDateNode
  type 'ABSOLUTEDATETIME_OCCURRED_AT'
...
#  do_not_validate_attachment_file_type :ingestedFileMetadata
  ...
  def self.ingestDayOneEntry(filename)
    ...
    if parsedEntry = Plist::parse_xml(file)
      ...
      entryNode.absoluteDatetimeRecordedAt = thisEntryDatetimeSecond
      # puts "Adding DayOne's second to Tree"
      # Example: 20151122_022100 > 20151122_0221 > 20151122_02 > 20151122 > 201511 > 2015
      entryNode.absoluteDatetimeRecordedAt.first.createAbsoluteDatetimeTreeFromHere
...
# puts "Creating geospatial Tree"
        entryNode.geospatialRecordedAt.first.createGeospatialTreeFromHere
      end
